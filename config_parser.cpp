...
std::tuple<ReturnCode, KeyValueMap> Parser::getSection(const std::string& section)
{
    ...
    if (it == sections.end())
    {
        ...
        return std::make_tuple(ReturnCode::SECTION_NOT_FOUND, keyValues);
    }
                                                    ...
                                                    const std::string& key)
{
    ...
    {
        return std::make_tuple(rc, values);
    }

    auto it = keyValues.find(key);
    if (it == keyValues.end())
    {
        return std::make_tuple(ReturnCode::KEY_NOT_FOUND, values);
    }
    ...
    return std::make_tuple(ReturnCode::SUCCESS, values);
}
