...
std::shared_ptr<phosphor::network::HypIPAddress> HypEthInterface::getIPAddrObject(std::string attrName, std::string oldIpAddr = "")
{
...
void HypEthInterface::setIpPropsInMap(std::string attrName, std::variant<std::string, int64_t> attrValue, std::string attrType)
{
...
void HypEthInterface::setBiosPropInDbus(std::shared_ptr<phosphor::network::HypIPAddress> ipObj, std::string attrName, std::variant<std::string, uint8_t> attrValue)
{
...
void HypEthInterface::watchBaseBiosTable()
{
    auto BIOSAttrUpdate = [&](sdbusplus::message::message& m) {
        std::map<std::string, std::variant<BiosBaseTableType>> interfacesProperties;
        ...
        {
            log<level::INFO>("BaseBIOSTable property not found. Continuing to listen...");
            return;
        }
        ...
        for (const BiosBaseTableItemType& attr : *baseBiosTable)
        {
            ...
            if (boost::starts_with(attrName, "vmi"))
            {
                ...
                if (boost::ends_with(attrName, "length") || boost::ends_with(attrName, "count"))
                {
                    auto findAttr = biosTableAttrs.find(attrName);
                    if (findAttr != biosTableAttrs.end())
                    {
                        const int64_t* attrValue = std::get_if<int64_t>(&std::get<biosBaseCurrValue>(attr.second));
                        if (std::get<int64_t>(findAttr->second) != *attrValue)
                        {
                            manager.setBIOSTableAttrs(attrName, *attrValue, "Integer");
                            ...
                            auto prefixItr = boost::algorithm::find_nth(attrName, "_", 2);
                            std::string hypPrefix = std::string(attrName.begin(), prefixItr.begin());
                            ...
                            if (ipObj != NULL)
                            {
                                ...
                                {
                                    setBiosPropInDbus(ipObj, "PrefixLength", static_cast<uint8_t>(*attrValue));
                                }
                            }
                            break;
                        }
                ...
                else 
                {
                    ...
                    if (findAttr != biosTableAttrs.end())
                    {
                        const std::string* attrValue = std::get_if<std::string>(&std::get<biosBaseCurrValue>(attr.second));
                        if (std::get<std::string>(findAttr->second) != *attrValue)
                        {
                            manager.setBIOSTableAttrs(attrName, *attrValue, "String");

                            auto prefixItr = boost::algorithm::find_nth(attrName, "_", 2);
                            std::string hypPrefix = std::string(attrName.begin(), prefixItr.begin());
                            hypPrefix.append("_ipaddr");

                            std::shared_ptr<phosphor::network::HypIPAddress> ipObj = NULL;

                            if (boost::ends_with(attrName, "ipaddr"))
                            {
                                ipObj = getIPAddrObject(hypPrefix, std::get<std::string>(findAttr->second));
                            }
                            else
                            {
                            ...
                            if (ipObj != NULL)
                            {
                                ...
                                setBiosPropInDbus(ipObj, dbusAttrName, *attrValue);
    ...
    phosphor::network::matchBIOSAttrUpdate = std::make_unique<sdbusplus::bus::match::match>(
        bus,
...
{
    // Access the biosTableAttrs of the parent object to create the ip address object
    ...
    for (std::string protocol : {"ipv4", "ipv6"})
    {
        ...
        if (biosTableItr != biosTableAttrs.end())
        {
            ipPrefixLength = static_cast<uint8_t>(std::get<int64_t>(biosTableItr->second));
        }
        ...
        addrs.emplace(ipAddr,
                      std::make_shared<phosphor::network::HypIPAddress>(
                          bus, (objectPath + "/ipv4/" + ipObjId).c_str(), *this,
                          ipProtocol, ipAddr, ipOrigin,
                          ipPrefixLength, ipGateway, intfLabel));
