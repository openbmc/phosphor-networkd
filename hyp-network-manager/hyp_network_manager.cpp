...
auto HypNetworkMgr::makeDBusCall(const std::string& objectName, const std::string& interface, const std::string& kw)
{
    auto bus = sdbusplus::bus::new_default();
    auto properties =
        bus.new_method_call("xyz.openbmc_project.BIOSConfigManager", objectName.c_str(),
...
void HypNetworkMgr::setBIOSTableAttrs(std::string attrName, std::variant<std::string, int64_t> attrValue, std::string attrType)
{
...
void HypNetworkMgr::setBIOSTableAttrs()
{
    try
    {
        ...
        makeDBusCall("/xyz/openbmc_project/bios_config/manager", "xyz.openbmc_project.BIOSConfig.Manager", "BaseBIOSTable").read(response);
        ...
        for (const BiosBaseTableItemType& item : *baseBiosTable)
        {
            if (boost::starts_with(item.first, "vmi"))
            {
                ...
                {
                    const int64_t* currValue = std::get_if<int64_t>(&std::get<biosBaseCurrValue>(item.second));
                ...
                {
                    const std::string* currValue = std::get_if<std::string>(&std::get<biosBaseCurrValue>(item.second));
                    biosTableAttrs.emplace(item.first, *currValue);
                }
            }
        }
