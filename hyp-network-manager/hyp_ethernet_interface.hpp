...
using CreateIface = sdbusplus::server::object::object<sdbusplus::xyz::openbmc_project::Network::server::EthernetInterface,
...
class HypEthInterface : public CreateIface
{
  public:
     ...
     */
    HypEthInterface(sdbusplus::bus::bus& bus, const char* path, HypNetworkMgr& parent) :
     ...
     * @param[in] oldIpAddr - optional (needed in case of ip address change in bios table)
     *
     * @return pointer to the ip dbus object
     */
    std::shared_ptr<phosphor::network::HypIPAddress> getIPAddrObject(std::string attrName, std::string /*oldIpAddr*/);
     ...
     */
    void setBiosPropInDbus(std::shared_ptr<phosphor::network::HypIPAddress> ipObj, std::string attrName, std::variant<std::string, uint8_t> attrValue);
     ...
     */
    void setIpPropsInMap(std::string attrName, std::variant<std::string, int64_t> attrValue, std::string attrType);
