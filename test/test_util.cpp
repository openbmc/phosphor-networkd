...
{
    std::string address = getNetwork(AF_INET,"9.3.23.251",24);
    EXPECT_EQ("9.3.23.0",address);

    address = getNetwork(AF_INET,"9.3.23.251",25);
    EXPECT_EQ("9.3.23.128",address);

    address = getNetwork(AF_INET6,"2001:db8:abcd:dd12::0",64);
    EXPECT_EQ("2001:db8:abcd:dd12::",address);

    address = getNetwork(AF_INET,"a.b.c.d",25);
    EXPECT_EQ("",address);

    address = getNetwork(AF_INET6,"2001:db8:gghh:dd12::0",64);
    EXPECT_EQ("",address);


    address = getNetwork(AF_INET6,"fe80::201:6cff:fe80:228",64);
