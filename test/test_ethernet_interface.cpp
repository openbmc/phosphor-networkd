...
TEST_F(TestEthernetInterface, DeleteNonStaticIPv4AddressNotAllowed)
{
    ...
    // flags=0 (no IFA_F_PERMANENT) → addAddr() falls through to dhcpIsEnabled(),
    // which returns true because the test fixture uses an empty config::Parser.
...
TEST_F(TestEthernetInterface, DeleteNonStaticIPv6AddressNotAllowed)
{
    ...
    // SLAAC branch, assigning AddressOrigin::SLAAC regardless of dhcpIsEnabled().
    interface.addAddr({.ifidx = 1,
