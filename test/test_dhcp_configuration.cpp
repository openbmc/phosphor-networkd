...
TEST_F(TestDHCPConfiguration, ReadValidConfig)
{
    ...
    std::ofstream file(testConfigFile);
    file << config.dump(4);
    file.close(); // Make sure file is written

    // Debug: Read and print file contents
    std::ifstream debugFile(testConfigFile);
    std::string contents((std::istreambuf_iterator<char>(debugFile)),
                         std::istreambuf_iterator<char>());
    std::cout << "File contents: " << contents << std::endl;

    auto parsedConfig = config::parseConfigFile(testConfigFile);
    std::cout << "Parsed config: " << parsedConfig.dump(4) << std::endl;
