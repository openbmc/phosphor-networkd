  ...
  public:
        ...
        stdplus::PinnedRef<sdbusplus::bus_t> bus, TestManager& manager)
    {
        ...
        return {bus, manager, info, "/xyz/openbmc_test/network",
                phosphor::network::config::Parser()};
    }
};
...
TEST_F(TestDHCPConfiguration, AllowConfigurationFalse)
{
    ...
    EXPECT_THROW(interface.dhcpEnabled(EthernetInterface::DHCPConf::v4),
                 NotAllowed);
    EXPECT_THROW(interface.dhcpEnabled(EthernetInterface::DHCPConf::v6),
                 NotAllowed);
    EXPECT_THROW(interface.dhcpEnabled(EthernetInterface::DHCPConf::both),
                 NotAllowed);
    EXPECT_THROW(
        interface.dhcpEnabled(EthernetInterface::DHCPConf::v4v6stateless),
        NotAllowed);
    EXPECT_THROW(
        interface.dhcpEnabled(EthernetInterface::DHCPConf::v6stateless),
        NotAllowed);
}
...
{
    std::filesystem::remove(testConfigFile);
    EXPECT_NO_THROW(interface.dhcpEnabled(EthernetInterface::DHCPConf::v4));
    EXPECT_NO_THROW(interface.dhcpEnabled(EthernetInterface::DHCPConf::v6));
    EXPECT_NO_THROW(interface.dhcpEnabled(EthernetInterface::DHCPConf::both));
    EXPECT_NO_THROW(
        interface.dhcpEnabled(EthernetInterface::DHCPConf::v4v6stateless));
    EXPECT_NO_THROW(
        interface.dhcpEnabled(EthernetInterface::DHCPConf::v6stateless));
