...
TEST_F(TestStaticGateway, MultipleIPv6Gateways)
{
    ...
    EXPECT_EQ(3, interface.staticGateways.size());
    EXPECT_EQ(IP::Protocol::IPv6, interface.staticGateways[gw1]->protocolType());
    EXPECT_EQ(IP::Protocol::IPv6, interface.staticGateways[gw2]->protocolType());
    EXPECT_EQ(IP::Protocol::IPv6, interface.staticGateways[gw3]->protocolType());
}
...
TEST_F(TestStaticGateway, MixedProtocolGateways)
{
    ...
    EXPECT_EQ(IP::Protocol::IPv4, interface.staticGateways[gw4]->protocolType());
    EXPECT_EQ(IP::Protocol::IPv6, interface.staticGateways[gw6]->protocolType());
}
...
TEST_F(TestStaticGateway, IPv4DifferentOctets)
{
    std::vector<std::string> gateways = {
        "1.2.3.4", "192.168.0.1", "172.16.0.1", "10.10.10.10"};
