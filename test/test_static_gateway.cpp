...
class TestStaticGateway : public stdplus::gtest::TestWithTmp
{
  public:
    ...
    TestManager manager;
    stdplus::Pinned<MockEthernetInterface> interface;

    TestStaticGateway() :
        bus(sdbusplus::bus::new_default()), confDir(CaseTmpDir()),
        manager(bus, "/xyz/openbmc_test/network", confDir),
        interface(bus, manager, makeInterfaceInfo(), "/xyz/openbmc_test/network"sv,
                  config::Parser())
    {}

    static AllIntfInfo makeInterfaceInfo()
    {
        return AllIntfInfo{InterfaceInfo{
            .type = ARPHRD_ETHER, .idx = 1, .flags = 0, .name = "test0"}};
    }

    std::unique_ptr<StaticGateway> createStaticGateway(std::string gateway,
...
{
    std::vector<std::string> gateways = {"2001:db8::1", "fe80::1", "::1",
