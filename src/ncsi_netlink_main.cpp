...
int main(int argc, char** argv)
{
    ...
    {
        const std::string& fcePayloadStr{options["flow-control"]};
        setNCSIFlowControlCmdPkt cmdPkt={0};
        int fceInt{};
        try
        {
        ...
        std::span<unsigned char> payload(
            std::bit_cast<unsigned char*>(&cmdPkt), sizeof(cmdPkt));
