...
static void printInfo(phosphor::network::ncsi::InterfaceInfo& info)
{
    ...
    {
        // Use std::cout for output to keep behavior aligned with
        // interface.getInfo(), instead of using lg2 logging.
        ...
        for (ChannelInfo& chan : pkg.channels)
        {
            ...
            std::cout << "      version " << chan.version_major << "."
                      << chan.version_minor << " (" << chan.version << ")"
                      << std::endl;

            std::cout << "      link state 0x" << std::hex << chan.link_state
                      << std::dec << std::endl;
