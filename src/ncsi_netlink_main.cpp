...
int main(int argc, char** argv)
{
    ...
    const std::string& macAddrArg{(options)["set-mac"]};
    if (!macAddrArg.empty())
    {
        if (macAddrArg.size() != 12)
            if (macAddrArg.size() < 12)
                exitWithError("Invalid MAC address: specify 12 hex digits.", argv);

        if (macAddrArg.size() > 12)
+           if (macAddrArg.size() != 14)
+                exitWithError("Specify 12 digit MAC address; (M/U); (E/D).", argv);

         std::string macAddrStr = macAddrArg;
+        std::transform(macAddrStr.begin(), macAddrStr.end(),
+                       macAddrStr.begin(), ::toupper);
+
...
+        macAddrStr.resize(12);
        ...
        return ncsi::setMacAddr(indexInt, channelInt,
