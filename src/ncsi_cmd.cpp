...
static int ncsiCommandReceiveDump(GlobalOptions& options, const std::string& subcommand, int argc, const char* const* argv)
{
    if (argc != 2)
    {
        std::cerr << "Invalid arguments for '" << subcommand << "' subcommand\n";
        print_usage(argv[0]); 
        return -1;
    }
    uint32_t handle = (subcommand == "core-dump") ? NCSI_CORE_DUMP_HANDLE : NCSI_CRASH_DUMP_HANDLE;
