...
static void print_usage(const char* progname)
{
    ...
    std::cerr
        ...
        "                           restricts the discovery to a specific package index.  (Note: if not provided for\n"
        "                           discovery, all packages will be scanned.)\n"
                       ...
                       std::vector<unsigned char> payload)
{
    ...
    auto ch = options.channel.transform([](unsigned int v) -> uint8_t {
...
static int ncsiDiscover(GlobalOptions& options)
{
    ...
    constexpr unsigned int maxPackageIndex = 7; // Packages 0–7
    constexpr unsigned int maxChannelCount = 8; // Channels 0–7
