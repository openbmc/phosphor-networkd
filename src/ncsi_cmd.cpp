...
static int ncsiCommand(GlobalOptions& options, uint8_t type,
                       std::vector<unsigned char> payload)
{
    NCSICommand cmd(type, static_cast<uint8_t>(*options.package), options.channel,
                    ...
                    const std::string& fileName)
{
    ...
    while (!isTransferComplete && !isAbort)
    {
        ...
        NCSICommand cmd(ncsiCmdDump, static_cast<uint8_t>(*options.package), options.channel, payload);
        auto resp = options.interface->sendCommand(cmd);
        if (!resp)
        {
    ...
    if (isAbort)
    {
        ...
        NCSICommand abortCmd(ncsiCmdDump, static_cast<uint8_t>(*options.package), options.channel,
                             abortPayload);
