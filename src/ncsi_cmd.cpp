...
static int ncsiDump(GlobalOptions& options, uint32_t handle, const std::string& fileName)
{
    ...
    if (handle != NCSI_CORE_DUMP_HANDLE && handle != NCSI_CRASH_DUMP_HANDLE) {
        std::cerr << "Invalid data handle value. Expected NCSI_CORE_DUMP_HANDLE (0xFFFF0000) or NCSI_CRASH_DUMP_HANDLE (0xFFFF0001), got: " << std::hex << handle << "\n";
        return -1;
    }
    ...
    while (!isTransferComplete)
    {
        ...
        std::vector<unsigned char> payload(payloadArray.begin(), payloadArray.end());

        NCSICommand cmd(ncsiCmdCoreDump, options.package, options.channel, payload);
        auto resp = options.interface->sendCommand(cmd);
        if (!resp)
        {
            std::cerr << "Failed to send NCSI command for chunk number " << chunkNum << "\n";
            return -1;
        }
        ...
        if (response != 0)
        {
            ...
            std::cerr << "Error  response code: " << response << ", reason code: " << reason << "\n";
	    ...
	    std::cerr << "opcode " << static_cast<int>(opcode) << "\n";
            ...
            if (outFile.is_open())
            {
                outFile.write(reinterpret_cast<const char*>(resp->payload.data() + 8), dataSize);
            }
            else
            {
        ...
        {
            std::cerr << "Received response with insufficient payload length: " << length << " Expected more than 8 bytes.\n";
        }
        ...
        if (resp->payload.size() == 8 && isAbort)
        {
            isTransferComplete = true;
        }
    }
