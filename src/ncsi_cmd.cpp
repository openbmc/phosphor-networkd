...
static int ncsiDiscover(GlobalOptions& options)
{
    ...
    for (unsigned int packageIndex = 0; packageIndex <= maxPackageIndex; ++packageIndex)
    {
        ...
        {
            std::cout << "  No valid response from Package " << packageIndex << ". Skipping.\n";
            continue; // Move to next package
        }

        uint8_t channelCount = resp->payload[47]; // Extract channel count
        ...
        for (unsigned int channelIndex = 1; channelIndex < channelCount; ++channelIndex)
        {
            ...
            // Get Capabilities command for each channel (starting from Channel 1)
            ...
            {
                std::cout << "  No valid response from Channel " << channelIndex << ". Skipping.\n";
                continue; // Move to next channel
            }

            // Found a valid package and channel, stop discovery
            std::cout << "  Found available Package " << packageIndex 
                      << ", Channel " << channelIndex << ". Stopping discovery.\n";
            return 0;
        }
    }
