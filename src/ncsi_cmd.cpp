                    ...
                    const std::string& fileName)
{
    ...
    if (isAbort)
    {
        ...
        std::vector<unsigned char> abortPayload(abortPayloadArray.begin(), abortPayloadArray.end());
        NCSICommand abortCmd(ncsiCmdDump, options.package, options.channel, abortPayload);
        auto abortResp = options.interface->sendCommand(abortCmd);
        if (!abortResp)
        {
            std::cerr << "Failed to send abort command for chunk number " << chunkNum << "\n";
        }
        else
        {
    ...
    std::cout << "Dump transfer complete. Total data size: " << totalDataSize << " bytes\n";
    outFile.close();
    return 0;
}
