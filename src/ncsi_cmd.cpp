    ...
    parseGlobalOptions(int argc, char* const* argv)
{
    ...
    if (!package.has_value() && !(optind < argc && std::string(argv[optind]) == "discover"))
    {
...
static int ncsiDiscover(GlobalOptions& options)
{
    ...
    while (pkg_idx <= 10) // Arbitrary package limit to prevent infinite loops
    {
        ...
        while (true)
        {
            std::cout << "Checking Package " << pkg_idx
                      << ", Channel " << channel_idx << "...\n";
            ...
            {
                std::cerr << "Invalid payload size in Get Capabilities response.\n";
                return -1;
            }

            uint8_t channelCount = resp->payload[47]; // Channel count in byte 47
            ...
            std::cout << "  Found available Package " << pkg_idx
                      << ", Channel " << channel_idx << ". Stopping discovery.\n";
            return 0;
