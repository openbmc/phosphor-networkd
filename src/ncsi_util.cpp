...
CallBack getVersionIdCallBack = [](struct nl_msg* msg, void* arg) {
    ...
    *(int*)arg = 0;
    ...
    getVersionIdResponse* getVersnIdResp = (getVersionIdResponse*)nla_data(tb[NCSI_ATTR_DATA]);

    auto ncsiVersn =
    std::span<const unsigned char>(reinterpret_cast<const unsigned char *>(&getVersnIdResp->versnIdData.ncsiVersion), 4);
    ...
    lg2::debug("PciVendor: {PCI_VENDOR_ID}", "PCI_VENDOR_ID", lg2::hex,
               getVersnIdResp->versnIdData.pciVenId);

    lg2::debug("PciDevice: {PCI_DEVICE_ID}", "PCI_DEVICE_ID", lg2::hex,
               getVersnIdResp->versnIdData.pciVenId);

    lg2::debug("Manufacturer Id (IANA): {MANUFACTURER_ID}", "MANUFACTURER_ID",
