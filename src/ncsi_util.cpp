...
constexpr auto NCSI_GET_CAPABILITIES = 0x16;

namespace internal
{

constexpr auto NUM_NCSI_REASON_CODE = 8;
constexpr auto REASON_NO_ERROR = 0x0000;
constexpr auto REASON_INTF_INIT_REQD = 0x0001;
constexpr auto REASON_PARAM_INVALID = 0x0002;
constexpr auto REASON_CHANNEL_NOT_RDY = 0x0003;
constexpr auto REASON_PKG_NOT_RDY = 0x0004;
constexpr auto REASON_INVALID_PAYLOAD_LEN = 0x0005;
constexpr auto REASON_INFO_NOT_AVAIL = 0x0006;
constexpr auto REASON_UNKNOWN_CMD_TYPE = 0x7FFF;
...
std::array<const char*, RESP_MAX> NCSIrespString = {
    "COMMAND_COMPLETED",
    "COMMAND_FAILED",
    "COMMAND_UNAVAILABLE",
    "COMMAND_UNSUPPORTED",
...
std::array<const char*, NUM_NCSI_REASON_CODE> NCSIreasonString = {
...
void printNCSICapabilities(const NCSIGetCapabilitiesResponse* capRcv)
{
    ...
    std::cout
        ...
        << "  unicast_filter_cnt = "
        << (NCSIGetCapabilitiesInfo::UNICAST_FILTER_MASK(capRcv->capabilitiesInfo.filterCnt))
        << std::endl
        << "  multicast_filter_cnt = "
        << (NCSIGetCapabilitiesInfo::MULTICAST_FILTER_MASK(capRcv->capabilitiesInfo.filterCnt))
        << std::endl
        << "  mixed_filter_cnt = "
        << (NCSIGetCapabilitiesInfo::MIXED_FILTER_MASK(capRcv->capabilitiesInfo.filterCnt)) << std::endl
        << "  vlan_filter_cnt = "
        << (NCSIGetCapabilitiesInfo::VLAN_FILTER_MASK(capRcv->capabilitiesInfo.filterCnt)) << std::endl
...
CallBack getCapabilitiesCallBack = [](struct nl_msg* msg, void* arg) {
    ...
    *(int*)arg = 0;
    ...
    std::array<unsigned char, sizeof(NCSIGetCapabilitiesResponse)> nlpayload = {};
    std::copy(data, data + nlpayload.size(), nlpayload.begin());

    NCSIGetCapabilitiesResponse capabilitiesResp =
        std::bit_cast<NCSIGetCapabilitiesResponse>(nlpayload);
