...
{
    uint8_t *pbuf = respBufCode;
    struct NCSICompletionCodes code;
    code.completionCodeResponse = (pbuf[0]<<8)+pbuf[1];
    code.completionCodeReason = (pbuf[2]<<8)+pbuf[3];
    lg2::debug("NCSI Response Code : {RESPONSE_CODE}",
               "RESPONSE_CODE", lg2::hex, code.completionCodeResponse);
    lg2::debug("NCSI Reason Code : {REASON_CODE}",
               "REASON_CODE", lg2::hex, code.completionCodeReason);
    return;
}
...
void printLinkStatus(unsigned char* msgdata)
{
    ...
    LinkStatus linkstatus;
    linkstatus.link = ntohl(plink->linkStatus.link);
    ...
    {
        lg2::debug("Speed and duplex : {SPEED_DUPLEX_STR}",
                   "SPEED_DUPLEX_STR",linkSpeedString[speed_duplex]);
    }
...
int printNCSIResponse(int ncsiRespType,int ncsiRespLen, unsigned char* respBuf) {
    ...
    default:
          ...
          auto str = toHexStr(std::span<const unsigned char>(NCSIMsgData,ncsiRespLen));
          lg2::debug("Command Specific Response {DATA_LEN} bytes: {DATA}", "DATA_LEN", ncsiRespLen,
               "DATA", str);
...
CallBack sendCntrlCallBack = [](struct nl_msg* msg, void* arg) {
    ...
    *(int*)arg = 0;
    ...
    NCSIPacketHeader * ncsiRespHdr = (NCSIPacketHeader *)nla_data(tb[NCSI_ATTR_DATA]);
    ...
    lg2::debug("NCSI Response length : {RESPONSE_LEN}",
               "RESPONSE_LEN", lg2::hex,respHdrLen);
                      ...
                      std::span<const unsigned char> payload)
{
    ...
    return internal::applyCmd(
        ifindex, internal::Command(ncsi_nl_commands::NCSI_CMD_SEND_CMD,
        ncsiCntl, payload), package, channel, NONE, internal::sendCntrlCallBack);
}
