...
static void printNCSIControllerPacketStats(NCSIControllerPacketStatsResponse &pResp)
{
    setlocale(LC_ALL, "");
    std::cout
        << "\nNIC statistics: "
...
CallBack statsCallback = [](struct nl_msg* msg, void* arg) {
    ...
    *(int*)arg = 0;
    ...
    auto endianCorrect = convertStatsToHostEndianess(reinterpret_cast
                            <NCSIControllerPacketStatsResponse*>(nla_data(tb[NCSI_ATTR_DATA])));
