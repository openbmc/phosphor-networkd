...
static NCSIControllerPacketStatsResponse convertStatsToHostEndianess(NCSIControllerPacketStatsResponse *inVar)
{
    ...
    localVar.excessiveCollisionFrames =
        ntohl(inVar->excessiveCollisionFrames);
...
static void printNCSIControllerStats(const NCSIControllerPacketStatsResponse &pResp)
{
    setlocale(LC_ALL, "");
    std::cout
        << "\nNIC statistics: "
...
CallBack statsCallback = [](struct nl_msg* msg, void* arg) {
    ...
    *(int*)arg = 0;
    ...
    printNCSIControllerStats(convertStatsToHostEndianess
                (reinterpret_cast<NCSIControllerPacketStatsResponse*>(nla_data(tb[NCSI_ATTR_DATA]))));
