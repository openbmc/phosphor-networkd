...
CallBack NcsiCoreDumpCallBack = [](struct nl_msg* msg, void* arg) {
    ...
    *(int*)arg = 0;
    ...
    lg2::debug("NcsiCoreDumpResponsePacket Info length: {LENGTH}, response: {RESPONSE}, "
           "reason: {REASON}, opcode: {OPCODE}",
           "LENGTH", length, "RESPONSE", lg2::hex, response,
           "REASON", lg2::hex, reason, "OPCODE", lg2::hex, opcode);

    if (response != 0) // Check response code for errors
    {
	lg2::error("Response error detected, reason code: {REASON}", "REASON", lg2::hex, reason);
	return -1;
    }
    ...
    {
        lg2::error("Received response with insufficient data length: {LENGTH}. Expected more than 8 bytes.", "LENGTH", length);
        return -1;
    }
...
int fileDump(int ifindex, int package, int channel, const std::string& fileName, const std::string& dumpType)
{
    ...
    while (!isTransferComplete)
    {
        ...
        lg2::debug("Send NCSI Command, CHANNEL : {CHANNEL} , PACKAGE : {PACKAGE}, "
                   ...
                   "CHANNEL", lg2::hex, channel, "PACKAGE", lg2::hex, package,
                   "INTERFACE_INDEX", lg2::hex, ifindex, "NCSI_CMD", lg2::hex, ncsiCmdCoreDump,
                   "CHUNK_NUM", chunkNum);
        lg2::debug("Payload: {PAYLOAD}", "PAYLOAD", toHexStr(payload));

        int result = internal::applyCmd(ifindex, internal::Command(ncsi_nl_commands::NCSI_CMD_SEND_CMD,
                                      ncsiCmdCoreDump, payload),
                                      package, channel, NONE, internal::NcsiCoreDumpCallBack);

        if (result < 0)
        {
            lg2::error("Error sending command for chunk number {CHUNK_NUM}, stopping", "CHUNK_NUM", chunkNum);
            break;
        }

        // Debug the opcode before deciding the next action
        lg2::debug("Received Opcode: {OPCODE}", "OPCODE", lg2::hex, internal::opcode);
        ...
        switch (internal::opcode)
        {
            ...
            case 0x8:  // Abort transfer
                ...
                break;
            default:
                lg2::error("Unexpected opcode: {OPCODE}", "OPCODE", internal::opcode);
