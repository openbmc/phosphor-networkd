...
void printNCSIGetParam(NCSIGetParametersResponse& pResp)
{
    std::cout
        ...
        << static_cast<int>(pResp.vlanMode) << "\n  aenControl = 0x" << std::hex
        << pResp.aenControl << "\n  mac1 = 0x" << std::hex << pResp.mac1
        << "\n  mac2 = 0x" << std::hex << pResp.mac2 << "\n  mac3 = 0x"
        << std::hex << pResp.mac3 << std::endl;
}

getParamResponse extractGetParameterData(const std::byte* data)
{
    getParamResponse paramResp;
    // Extract the header of the response
    paramResp.paramRespHdr.MCID = *std::bit_cast<uint8_t*>(data);
    data += sizeof(uint8_t);
    paramResp.paramRespHdr.revision = *std::bit_cast<uint8_t*>(data);
    data += sizeof(uint8_t);
    paramResp.paramRespHdr.reserved = *std::bit_cast<uint8_t*>(data);
    data += sizeof(uint8_t);
    paramResp.paramRespHdr.id = *std::bit_cast<uint8_t*>(data);
    data += sizeof(uint8_t);
    paramResp.paramRespHdr.type = *std::bit_cast<uint8_t*>(data);
    data += sizeof(uint8_t);
    paramResp.paramRespHdr.channel = *std::bit_cast<uint8_t*>(data);
    data += sizeof(uint8_t);
    paramResp.paramRespHdr.length = ntohs(*std::bit_cast<uint16_t*>(data));
    data += sizeof(uint16_t);
    data += sizeof(uint32_t) * 2; // skip reserved
...
CallBack paramCallback = [](struct nl_msg* msg, void* arg) {
    using namespace phosphor::network::ncsi;
    auto nlh = nlmsg_hdr(msg);
    std::array<struct nlattr*, NCSI_ATTR_MAX + 1> tb = {nullptr};
    constexpr struct nla_policy ncsiPolicy[NCSI_ATTR_MAX + 1] = {
        {NLA_UNSPEC, 0, 0}, {NLA_U32, 0, 0}, {NLA_NESTED, 0, 0},
        {NLA_U32, 0, 0},    {NLA_U32, 0, 0}, {NLA_BINARY, 0, 0},
        {NLA_FLAG, 0, 0},   {NLA_U32, 0, 0}, {NLA_U32, 0, 0},
    };
    ...
    *(int*)arg = 0;
    auto ret = genlmsg_parse(nlh, 0, tb.data(), NCSI_ATTR_MAX, ncsiPolicy);
    if (ret)
    {
        lg2::error("Failed to parse package");
        return ret;
    ...
    std::byte* data = std::bit_cast<std::byte*>(nla_data(tb[NCSI_ATTR_DATA]));
    auto paramResp = extractGetParameterData(data);

    lg2::debug("NCSI Response packet type : {RESPONSE_PKT_TYPE}",
               "RESPONSE_PKT_TYPE", lg2::hex, paramResp.paramRespHdr.type);
    lg2::debug("NCSI Response length : {RESPONSE_LEN}", "RESPONSE_LEN",
               lg2::hex, paramResp.paramRespHdr.length);

    lg2::debug("NCSI Response Code : {RESPONSE_CODE}", "RESPONSE_CODE",
               lg2::hex, paramResp.paramStatCode.completionCodeResponse);
    lg2::debug("NCSI Reason Code : {REASON_CODE}", "REASON_CODE", lg2::hex,
               paramResp.paramStatCode.completionCodeReason);

    printNCSIGetParam(paramResp.paramResp);
