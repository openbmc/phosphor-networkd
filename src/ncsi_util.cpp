...
struct NCSIpacketHeader
{
    ...
    uint8_t revision;
    uint8_t reserved2; // Reserved has to set to 0x00
    uint8_t id;
    uint8_t type;
    uint8_t channel;
...
void printNCSIcompletionCodes(const NCSIcompletionCodes* capComCodes)
{
    ...
    std::cout << "Response: " << NCSIccRespName(ccResp) << " (" << "0x" << std::hex
         << ccResp << ")" << std::endl;
    std::cout << "Reason: " << NCSIccResonName(ccReason) << " (" << "0x" << std::hex
         << ccReason << ")" << std::endl;
...
void printNCSIcapabilities(const NCSIgetCapabilitiesResponse* capRcv)
{
    ...
    std::cout << "cmd: " << NCSIcmdTypeToName(static_cast<int>(cmdSent)) << "("
         << "0x" << std::hex << static_cast<int>(cmdSent) << ")" << std::endl;
    ...
    if(static_cast<int>(cmdSent) != NCSI_GET_CAPABILITIES)
    {
        ...
        auto str = toHexStr(std::span<const unsigned char>(
            reinterpret_cast<const unsigned char*>(&capRcv->capComCodes), dataLen));
        lg2::debug("Response {DATA_LEN} bytes: {DATA}", "DATA_LEN", dataLen,
                   "DATA", str);
        return;
    }
    lg2::debug("Get Capabilities response:");
    std::cout << "  capabilities_flags = " << "0x" << std::hex
         ...
         << "  unicast_filter_cnt = "
         << (UNICAST_FILTER_MASK(capRcv->capabilitiesInfo.filterCnt)) << std::endl
         << "  multicast_filter_cnt = "
         << (MULTICAST_FILTER_MASK(capRcv->capabilitiesInfo.filterCnt)) << std::endl
         << "  mixed_filter_cnt = "
         << (MIXED_FILTER_MASK(capRcv->capabilitiesInfo.filterCnt)) << std::endl
         << "  vlan_filter_cnt = "
         << (VLAN_FILTER_MASK(capRcv->capabilitiesInfo.filterCnt)) << std::endl
         << "  channel_cnt = " << capRcv->capabilitiesInfo.channelCnt << std::endl
...
CallBack sendCallBack = [](struct nl_msg* msg, void* arg) {
    ...
    *(int*)arg = 0;
    ...
    lg2::debug("Response {DATA_LEN} bytes: {DATA}", "DATA_LEN", dataLen, "DATA",
               str);
