...
static constexpr uint32_t UNICAST_FILTER_MASK(uint32_t x) { return (((x) & 0xff000000UL) >> 24); }
static constexpr uint32_t MULTICAST_FILTER_MASK(uint32_t x) { return (((x) & 0x00ff0000UL) >> 16); }
static constexpr uint32_t MIXED_FILTER_MASK(uint32_t x) { return (((x) & 0x0000ff00UL) >> 8); }
static constexpr uint32_t VLAN_FILTER_MASK(uint32_t x) { return (((x) & 0x000000ffUL) >> 0); }
...
CallBack getCapabilitiesCallBack = [](struct nl_msg* msg, void* arg) {
    ...
    *(int*)arg = 0;
    ...
    NCSIGetCapabilitiesResponse* capabilitiesResp =
        reinterpret_cast<NCSIGetCapabilitiesResponse*>(nla_data(tb[NCSI_ATTR_DATA]));
