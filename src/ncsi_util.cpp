...
{
  public:
    NcsiDumpTransfer(const std::string& fileName) : totalDataSize(0), abort(false)
    {
    ...
    int callback(struct nl_msg* msg)
    {
        ...
        lg2::debug(
            ...
            "LENGTH", length, "RESPONSE", lg2::hex, response, "RESPONSE_DESC", responseDesc,
            "REASON", lg2::hex, reason, "OPCODE", lg2::hex, opcode);
        ...
        if (response != 0) // Check response code for errors
        {
            ...
            switch (reason)
            {
                case 0x0001:
                    ...
                    break;
                case 0x0002:
                    reasonDesc = "Parameter Is Invalid, Unsupported, or Out-of-Range";
                    break;
                case 0x0003:
                ...
                case 0x7FFF:
                    ...
                    break;
                case 0x4D01:
                    reasonDesc = "Abort Transfer: NC cannot proceed with transfer.";
                    break;
                case 0x4D02:
                    reasonDesc = "Invalid Handle Value: Data Handle is invalid or not supported.";
                    break;
                case 0x4D03:
                    reasonDesc = "Sequence Count Error: Chunk Number requested is not consecutive with the previous number transmitted.";
                    break;
                default:
            ...
            lg2::error("Response error detected, response code: {RESPONSE} ({RESPONSE_DESC}), reason code: {REASON} ({REASON_DESC})", "RESPONSE", lg2::hex, response, "RESPONSE_DESC", responseDesc, "REASON", lg2::hex, reason, "REASON_DESC", reasonDesc);
            return -1;
        }
        ...
        if (length > 8)
        {
            ...
            if (length % 4 != 0)
            {
                lg2::error("Packet length is not 4-byte aligned: {LENGTH}", "LENGTH", length);
                return -1;
            }
             ...
             const std::string& dataHandleStr)
{
    ...
    try
    {
        ...
        {
            lg2::error("Invalid data handle string: {DATA_HANDLE_STR}", "DATA_HANDLE_STR", dataHandleStr);
            return -1;
        }
        if (dataHandle > std::numeric_limits<uint32_t>::max())
        {
            lg2::error("Data handle out of range for uint32_t: {DATA_HANDLE}", "DATA_HANDLE", dataHandle);
            return -1;
        }
        // Check if the data handle matches known valid values
        if (dataHandle != internal::NCSI_CORE_DUMP_HANDLE && dataHandle != internal::NCSI_CRASH_DUMP_HANDLE)
        {
            lg2::error("Invalid data handle value. Expected NCSI_CORE_DUMP_HANDLE (0xFFFF0000) or NCSI_CRASH_DUMP_HANDLE (0xFFFF0001), got: {DATA_HANDLE}", "DATA_HANDLE", lg2::hex, dataHandle);
            return -1;
        }
    }
    catch (const std::invalid_argument& e)
    {
        lg2::error("Invalid argument for data handle conversion: {DATA_HANDLE_STR}", "DATA_HANDLE_STR", dataHandleStr);
        return -1;
    }
    catch (const std::out_of_range& e)
    {
        lg2::error("Data handle out of range: {DATA_HANDLE_STR}", "DATA_HANDLE_STR", dataHandleStr);
        return -1;
    }
    ...
    while (!isTransferComplete)
    {
        ...
        switch (transfer.getOpcode())
        {
            ...
            case 0x2: // Middle chunk
                if (chunkNum <= 1)
                {
                    lg2::error("Invalid chunk number for middle chunk: {CHUNK_NUM}", "CHUNK_NUM", chunkNum);
                    return -1;
                }
                chunkNum++; // Proceed to the next chunk
                break;
