...
public:
  ...
  Command(int ncsiCmd, int operation = DEFAULT_VALUE,
...
CallBack infoCallBack = [](struct nl_msg *msg, void *arg) {
  ...
  *(int *)arg = 0;
  ...
  nla_for_each_nested(attrTgt, tb[NCSI_ATTR_PACKAGE_LIST], rem) {
    ret =
        nla_parse_nested(packagetb, NCSI_PKG_ATTR_MAX, attrTgt, packagePolicy);
    if (ret < 0) {
    ...
    if (packagetb[NCSI_PKG_ATTR_ID]) {
      ...
      lg2::debug("Package has id : {ATTR_ID}", "ATTR_ID", lg2::hex, attrID);
    } else {
    ...
    auto channelListTarget =
        static_cast<nlattr *>(nla_data(packagetb[NCSI_PKG_ATTR_CHANNEL_LIST]));
    ...
    nla_for_each_nested(channelListTarget,
      ...
      if (channeltb[NCSI_CHANNEL_ATTR_ID]) {
        ...
        if (channeltb[NCSI_CHANNEL_ATTR_ACTIVE]) {
          lg2::debug("Channel Active : {CHANNEL}", "CHANNEL", lg2::hex,
                     channel);
        } else {
          lg2::debug("Channel Not Active : {CHANNEL}", "CHANNEL", lg2::hex,
                     channel);
        }
      ...
      if (channeltb[NCSI_CHANNEL_ATTR_LINK_STATE]) {
        ...
        lg2::debug("Channel Link State : {LINK_STATE}", "LINK_STATE", lg2::hex,
                   link);
      }
...
CallBack sendCallBack = [](struct nl_msg *msg, void *arg) {
  ...
  *(int *)arg = 0;
  ...
  lg2::debug("Response {DATA_LEN} bytes: {DATA}", "DATA_LEN", data_len, "DATA",
             str);
  printNCSIControllerStats(data);
...
int applyCmd(int ifindex, const Command &cmd, int package = DEFAULT_VALUE,
  ...
  if (package != DEFAULT_VALUE) {
    ret = nla_put_u32(msg.get(), ncsi_nl_attrs::NCSI_ATTR_PACKAGE_ID, package);
    if (ret < 0) {
  ...
  if (channel != DEFAULT_VALUE) {
    ret = nla_put_u32(msg.get(), ncsi_nl_attrs::NCSI_ATTR_CHANNEL_ID, channel);
    if (ret < 0) {
  ...
  if (cmd.operation != DEFAULT_VALUE) {
    ...
    ret =
        nla_put(msg.get(), ncsi_nl_attrs::NCSI_ATTR_DATA, pl.size(), pl.data());
    if (ret < 0) {
      lg2::error("Failed to set the data attribute, RC : {RC}", "RC", ret);
      return ret;
    }
...
int setChannel(int ifindex, int package, int channel) {
  lg2::debug("Set CHANNEL : {CHANNEL} , PACKAGE : {PACKAGE}, INTERFACE_INDEX: "
             "{INTERFACE_INDEX}",
             "CHANNEL", lg2::hex, channel, "PACKAGE", lg2::hex, package,
             "INTERFACE_INDEX", lg2::hex, ifindex);
  return internal::applyCmd(
...
void printNCSICompletionCodes(unsigned char *rcv_buf) {
  ...
  std::cout << "Response: 0x000" << cc_resp << "\t" << "Reason: 0x000"
            << cc_reason << std::endl;
...
void printNCSIControllerStats(unsigned char *rcv_buf) {
  ...
  std::cout << "Jabber Packets Received: " << ntohl(pResp->jabberPktsRcvd)
            << std::endl;
  std::cout << "Pause XON Frames Received: " << ntohl(pResp->pauseXOnFramesRcvd)
            << std::endl;
  ...
  std::cout << "65-127 Byte Frames Received: " << ntohl(pResp->rxFrame_65_127)
            << std::endl;
  std::cout << "128-255 Byte Frames Received: " << ntohl(pResp->rxFrame_128_255)
            << std::endl;
  std::cout << "256-511 Byte Frames Received: " << ntohl(pResp->rxFrame_256_511)
            << std::endl;
