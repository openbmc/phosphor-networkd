...
std::uint32_t getField(std::uint32_t value, std::uint32_t mask)
{
    // Set bit found return field (Least bit postion start with 1)
    // Set bit not found return 0
...
void printLinkStatus(const getLinkStatusResponse* linkRcv)
{
    ...
    lg2::debug("Link Status : Link is {UP_DOWN}", "UP_DOWN",
               getField(linkRcv->linkStatus, linkFlagMask) ? "Up" : "Down");

    speed_duplex = ((getField(linkRcv->linkStatus, speedDuplexMask)) == useExt)
                       ? getField(linkRcv->linkStatus, extSpeedDuplexMask)
                       : getField(linkRcv->linkStatus, speedDuplexMask);
    ...
    if (getField(linkRcv->linkStatus, autoNegotiateEnMask))
    {
        lg2::debug("Auto-negotiation : Enabled");
        lg2::debug("Auto-negotiation completed : {YES_NO}", "YES_NO",
                   getField(linkRcv->linkStatus, autoNegoationCompleteMask)
                       ? "Yes"
                       : "No");
    }
    else
    ...
    lg2::debug("Parallel Detection : {USED_NOTUSED}", "USED_NOTUSED",
               getField(linkRcv->linkStatus, parallelDetectionMask)
                   ? "Used"
                   : "Not used");

    lg2::debug("TX Flow Control : {ENABLE_DISABLE}", "ENABLE_DISABLE",
               getField(linkRcv->linkStatus, txFlowControlMask) ? "Enabled"
                                                                : "Disabled");

    lg2::debug("RX Flow Control : {ENABLE_DISABLE}", "ENABLE_DISABLE",
               getField(linkRcv->linkStatus, rxFlowControlMask) ? "Enabled"
                                                                : "Disabled");

    lg2::debug("SerDes Status : {USED_NOTUSED}", "USED_NOTUSED",
               getField(linkRcv->linkStatus, serdesMask)
    ...
    lg2::debug("OEM Link Speed setting : {VALID_INVALID}", "VALID_INVALID",
               getField(linkRcv->linkStatus, oemLinkSpeedValidMask)
                   ? "Valid"
                   : "Invalid");

    if (getField(linkRcv->linkStatus, modulationSchemeMask) == useNRZ)
    {
        lg2::debug("Modulation Scheme : NRZ");
    }
    else if (getField(linkRcv->linkStatus, modulationSchemeMask) == usePAMFour)
    {
    ...
    if (!getField(linkRcv->linkStatus, serdesMask) &&
        getField(linkRcv->linkStatus, autoNegotiateEnMask) &&
        getField(linkRcv->linkStatus, autoNegoationCompleteMask))
    {
        lg2::debug("Link Partner Advertised Settings :");
        lg2::debug("    Speed and Duplex 1000TFD : {CAPABILITY}", "CAPABILITY",
                   getField(linkRcv->linkStatus, linkPartner1000FullDuplexMask)
                       ? "Capable"
                       : "Not capable");

        lg2::debug("    Speed and Duplex 1000THD : {CAPABILITY}", "CAPABILITY",
                   getField(linkRcv->linkStatus, linkPartner1000HalfDuplexMask)
                       ? "Capable"
                       : "Not capable");

        lg2::debug("    Speed and Duplex 100T4 : {CAPABILITY}", "CAPABILITY",
                   getField(linkRcv->linkStatus, linkPartner100T4Mask)
                       ? "Capable"
                       : "Not capable");

        lg2::debug("    Speed and Duplex 100TXFD : {CAPABILITY}", "CAPABILITY",
                   getField(linkRcv->linkStatus, linkPartner100FullDuplexMask)
                       ? "Capable"
                       : "Not capable");

        lg2::debug("    Speed and Duplex 100TXHD : {CAPABILITY}", "CAPABILITY",
                   getField(linkRcv->linkStatus, linkPartner100HalfDuplexMask)
                       ? "Capable"
                       : "Not capable");

        lg2::debug("    Speed and Duplex 10TFD : {CAPABILITY}", "CAPABILITY",
                   getField(linkRcv->linkStatus, linkPartner10FullDuplexMask)
                       ? "Capable"
                       : "Not capable");

        lg2::debug("    Speed and Duplex 10THD : {CAPABILITY}", "CAPABILITY",
                   getField(linkRcv->linkStatus, linkPartner10HalfDuplexMask)
                       ? "Capable"
                       : "Not capable");

        if (getField(linkRcv->linkStatus, linkPartnerFlowControlMask) == useSym)
        {
            lg2::debug("    LinkPartner FlowControl : Symmetric");
        }
        else if (getField(linkRcv->linkStatus, linkPartnerFlowControlMask) ==
                 useAsym)
        {
