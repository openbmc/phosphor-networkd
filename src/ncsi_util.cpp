             ...
             CallBack function = nullptr)
{
    ...
    {
        if (cmd.payload.size() != sizeof(uint32_t))
        {
        ...
        ret = nla_put_u32(
            msg.get(), maskAttr,
            *(reinterpret_cast<const uint32_t*>(cmd.payload.data())));
        if (ret < 0)
        {
...
int setPackageMask(int ifindex, std::span<const unsigned char> payload)
{
    lg2::warning("Set Package Mask , INTERFACE_INDEX: {INTERFACE_INDEX} ",
                 "INTERFACE_INDEX", lg2::hex, ifindex);
    return internal::applyCmd(
...
int setChannelMask(int ifindex, int package,
                   std::span<const unsigned char> payload)
{
    lg2::warning(
        "Set Channel Mask , INTERFACE_INDEX: {INTERFACE_INDEX}, PACKAGE : {PACKAGE}",
