...
void executeCommandinChildProcess(stdplus::const_zstring path, char** args)
{
    ...
    else if (pid > 0)
    {
        ...
        if (status < 0)
        {
            ...
            buf.push_back('\0');
            error("Unable to execute the command {CMD}: {STATUS}", "CMD",
                             ...
                             auto&& fun)
{
    if (!config.getFileExists())
    {
             ...
             str == nullptr)
    {
        notice("Unable to get the value of {SECTION}[{KEY}] from {FILE}",
               "SECTION", section, "KEY", key, "FILE", config.getFilename());
    }
    else if (auto val = fun(*str); !val)
    {
        notice("Invalid value of {SECTION}[{KEY}] from {FILE}: {VALUE}",
               "SECTION", section, "KEY", key, "FILE", config.getFilename(),
               "VALUE", *str);
    }
    else
