    ...
    bus(bus), event(event)
{
    ...
    configs = phosphor::lldp_utils::LLDPUtils::parseAllConfigs(
        "/etc/lldpd.conf", "/etc/lldpd.d");
}
...
{
    std::vector<std::string> matchPrefix = {"configure", "ports", iface, "lldp",
                                            "status"};
...
{
    std::vector<std::string> matchPrefix = {"configure", "ports", ifname,
                                            "lldp", "status"};
