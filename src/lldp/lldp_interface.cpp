...
void Interface::refreshInterface()
{
    ...
    lldpctl_atom_foreach(ports, port)
    {
        ...
        lldpctl_atom_foreach(neighbors, neigh)
        {
            ...
            if (mgmtMac.empty() &&
                chassisSubtypeStr && std::string_view(chassisSubtypeStr) == "mac")
            {
            ...
            updateOrCreateReceiveObj(
                ...
                sysdesc ? std::string(sysdesc) : std::string(), mgmtV4, mgmtV6, mgmtMac);
        }
    ...
    const std::string& mgmtMac)
{
    ...
    if (receive)
    {
        ...
        if (!isSameNeighbor)
        {
            // If TLV values are changed, remove existing receive object before recreating
            lg2::info(
        ...
        else
        {
            ...
            bool otherPropChanged = (tlv.TLVsIface::systemName() != sysName) ||
    ...
    try
    {
        auto tlvObj = std::make_unique<TLVs>(
                        bus, path,
                        chassisId.empty() ? "": chassisId,
                        TLVsIface::IEEE802IdSubtype::NotTransmitted,
                        portId.empty() ? "": portId,
                        TLVsIface::IEEE802IdSubtype::NotTransmitted,
                        sysName.empty() ? "": sysName,
                        sysDesc.empty() ? "": sysDesc,
                        std::vector<TLVsIface::SystemCapabilities>(),
                        mgmtIPv4.empty() ? "": mgmtIPv4,
                        mgmtIPv6.empty() ? "": mgmtIPv6,
                        mgmtMac.empty() ? "": mgmtMac,
                        0,
