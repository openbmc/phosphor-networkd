                                        ...
                                        std::string_view data)
{
    switch (hdr.nlmsg_type)
    {
        case RTM_NEWLINK:
        case RTM_DELLINK:
            return std::make_optional(extractRtData<ifinfomsg>(data).ifi_index);
        case RTM_NEWADDR:
        case RTM_DELADDR:
            return std::make_optional(extractRtData<ifaddrmsg>(data).ifa_index);
        case RTM_NEWNEIGH:
        case RTM_DELNEIGH:
            return std::make_optional(extractRtData<ndmsg>(data).ndm_ifindex);
        default:
            return std::nullopt;
    }
}
...
static void handler(Manager& m, const nlmsghdr& hdr, std::string_view data)
{
    ...
    catch (const std::exception& e)
    {
        try
        {
            ...
            if (ifIdx && m.ignoredIntf.contains(*ifIdx))
            {
