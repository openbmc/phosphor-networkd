...
void EthernetInterface::addStaticRoute(const StaticRouteInfo& info)
{
    ...
    if (auto it = staticRoutes.find(*info.destination); it != staticRoutes.end())
    {
        it->second->StaticRouteObj::gateway(*info.gateway);
    }
    else
    {
        staticRoutes.emplace(*info.destination, std::make_unique<StaticRoute>(
                                                bus, std::string_view(objPath),
                                                *this, *info.destination, *info.gateway,
                                                info.prefixLength));
    }
}
                                       ...
                                       std::string gateway, uint8_t prefixLength)
{
    ...
    {
        auto msg =
            fmt::format("Not a valid IP address `{}`: {}", destination, e.what());
    ...
    if (it == staticRoutes.end())
    {
        it = std::get<0>(staticRoutes.emplace(
             destination, std::make_unique<StaticRoute>(bus, std::string_view(objPath),
                                             *this, destination, gateway,
                                             prefixLength)));
    }
    else
    {
        //auto str = std::to_string(destination);
        if (it->second->StaticRouteObj::gateway() == destination)
        {
            return it->second->getObjPath();
        }
...
void EthernetInterface::writeConfigurationFile()
{
...

    {
        ...
        for (const auto& temp : staticRoutes)
        {
            ...
            staticRoute["Destination"].emplace_back(fmt::format("{}/{}", temp.second->destination(), temp.second->prefixLength()));
