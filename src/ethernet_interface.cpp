...
EthernetInterface::DHCPConf EthernetInterface::dhcpEnabled(DHCPConf value)
{
    ...
    auto newra = EthernetInterfaceIntf::ipv6AcceptRA(
        value == DHCPConf::v4v6stateless || value == DHCPConf::v6 ||
        value == DHCPConf::both);
...
void EthernetInterface::writeConfigurationFile()
{
    ...
    auto file = config::pathForIntfConf(manager.getConfDir(), interfaceName());
    config.writeFile(file);
    auto msg = fmt::format("Wrote networkd file: {}", file.native());
    log<level::INFO>(msg.c_str(), entry("FILE=%s", file.c_str()));
}
