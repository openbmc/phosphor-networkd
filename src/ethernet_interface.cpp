...
void EthernetInterface::writeConfigurationFile()
{
	...
	lg2::info("dhcp4 dns_enabled {DNS_ENABLED}", "DNS_ENABLED", dns_enabled);
        dhcp4["UseNTP"].emplace_back(conf.ntpEnabled() ? "true" : "false");
	lg2::info("dhcp4 ntpEnabled {NTPENABLED}", "NTPENABLED", conf.ntpEnabled());
        dhcp4["UseHostname"].emplace_back(conf.hostNameEnabled() ? "true"
                                                                 : "false");
	lg2::info("dhcp4 hostNameEnabled {HOSTNAMEENAMBLED}", "HOSTNAMEENAMBLED", conf.hostNameEnabled());
	...
	lg2::info("dhcp6 dns_enabled {DNS_ENABLED}","DNS_ENABLED",  dns_enabled);
        dhcp6["UseNTP"].emplace_back(conf.ntpEnabled() ? "true" : "false");
	lg2::info("dhcp6 ntpEnabled {NTPENABLED}","NTPENABLED", conf.ntpEnabled());
        dhcp6["UseHostname"].emplace_back(conf.hostNameEnabled() ? "true"
                                                                 : "false");
	lg2::info("dhcp6 hostNameEnabled {HOSTNAMEENAMBLED}","HOSTNAMEENAMBLED", conf.hostNameEnabled());
    }
