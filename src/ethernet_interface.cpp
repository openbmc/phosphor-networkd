        ...
        }
        {
            if (!dhcp4())
            {
                ...
                if (!gateway4.empty())
                {
                    ...
                    std::string routeAddressPrefix = setIPv4AddressLastOctetToZero(gateway4);
                    routeAddressPrefix = routeAddressPrefix + "/" +std::to_string(prefixLength);
                    auto& routingPolicyTo = config.map["RoutingPolicyRule"].emplace_back();
                    routingPolicyTo["Table"].emplace_back(routingTableId);
                    routingPolicyTo["To"].emplace_back(routeAddressPrefix);
                    auto& routingPolicyFrom = config.map["RoutingPolicyRule"].emplace_back();
