...
void EthernetInterface::writeConfigurationFile()
{
    ...
    {
        auto dir = config::pathForIntfConf("/run/systemd/network"sv, interfaceName()) + ".d";
        auto file = dir + "/updated.conf";
        try
        {
            std::filesystem::create_directories(dir);
            using namespace stdplus::fd;
            open(file, OpenFlags(OpenAccess::WriteOnly).set(OpenFlag::Create), 0644);
        }
        catch (const std::exception &e)
        {
            lg2::error("Failed to write networkd timestamp file {FILE}: {ERROR}", "FILE", "ERROR", e.what());
        }
    }
}
