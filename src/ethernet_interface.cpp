...
// DHCP:
//      Only ipv4 or false to control IPv4 DHCP, IPv6 dyanmic addressing is
//      controlled explicitly via IPv6AcceptRA and DHCPv6Client
// IPv6AcceptRA:
//      true: respond to a Routing Advertisement server on the local link
//      false: ignore Routing Advertisement servers. IPv6 addresses must be
//      statically assigned.
// DHCPv6Client:
//      true: respond to DHCPv6 servers on the local link
//      false: ignore DHCPv6 servers on the local link, unless the RA is
//      configured to override via the "Other" bit
std::map<EthernetInterface::DHCPConf,
         std::tuple<std::string, std::string, std::string>>
    mapDHCPToSystemd = {
        {EthernetInterface::DHCPConf::both, {"ipv4", "true", "true"}},
        {EthernetInterface::DHCPConf::v4v6stateless, {"ipv4", "true", "false"}},
