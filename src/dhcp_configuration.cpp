    ...
    parent(parent)
{
    ...
    ConfigIntf::dnsEnabled(getDHCPProp(conf, "UseDNS", type), true);
    ConfigIntf::ntpEnabled(getDHCPProp(conf, "UseNTP", type), true);
    ConfigIntf::hostNameEnabled(getDHCPProp(conf, "UseHostname", type), true);
    if (type == NetworkType::dhcp4)
    {
        ConfigIntf::sendHostNameEnabled(
            getDHCPProp(conf, "SendHostname", type, true);
    }
