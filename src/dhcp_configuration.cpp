    ...
    parent(parent)
{
    ...
    ConfigIntf::hostNameEnabled(getDHCPProp(conf, "UseHostname", type.c_str()), true);
    ConfigIntf::sendHostNameEnabled(getDHCPProp(conf, "SendHostname", type.c_str()), true);
    emit_object_added();
}
...
bool Configuration::sendHostNameEnabled(bool value)
{
    ...
    lg2::info("SendHostNameEnabled {VALUE}", "VALUE", value);
    return name;
}
...
bool Configuration::hostNameEnabled(bool value)
{
    ...
    parent.get().writeConfigurationFile();
    parent.get().manager.get().reloadConfigs();
    lg2::info("HostNameEnabled {VALUE}", "VALUE", value);
...
bool Configuration::ntpEnabled(bool value)
{
    ...
    parent.get().writeConfigurationFile();
    parent.get().manager.get().reloadConfigs();
    lg2::info("ntpEnabled {VALUE}", "VALUE", value);
...
bool Configuration::dnsEnabled(bool value)
{
    ...
    parent.get().writeConfigurationFile();
    parent.get().manager.get().reloadConfigs();
    lg2::info("dnsEnabled {VALUE}", "VALUE", value);
