...
static constexpr const char* mapperObjPath = "/xyz/openbmc_project/object_mapper";
...
std::string HostnameManager::getCurrentHostname()
{
    try
    {
        auto method = bus.get().new_method_call(hostnamedBusName,
                                                hostnamedObjPath, propIntf,
                                                "Get");
...
bool HostnameManager::setHostname(const std::string& hostname)
{
    try
    {
        auto method = bus.get().new_method_call(
            hostnamedBusName, hostnamedObjPath, hostnamedIntf,
            "SetStaticHostname");
