...
class HypEthInterface : public CreateIface
{
  public:
    ...
    template <typename Addr>
    static void validateGateway(std::string& gw, const std::string& localIP = "")
    {
        try
        {
            ...
            // Check if gateway is the same as local IP
            if (!localIP.empty())
            {
                auto localAddr = stdplus::fromStr<Addr>(localIP);
                if (ip == localAddr)
                {
                    throw std::invalid_argument(
                        "Gateway cannot be the same as local IP address");
                }
            }
            gw = stdplus::toStr(ip);
        }
