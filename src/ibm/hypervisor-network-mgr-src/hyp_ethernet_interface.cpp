...
void HypEthInterface::createIPAddressObjects()
{
    ...
    if (biosTableAttrs.size() < BIOS_ATTRS_SIZE)
    {
        ...
        if (intfLabel == "if0")
        {
            ...
            addrs.emplace("eth0/v4", std::make_unique<HypIPAddress>(
                                      bus,
            ...
            addrs.emplace("eth0/v6", std::make_unique<HypIPAddress>(
                                      bus,
                                      sdbusplus::message::object_path(
        ...
        else if (intfLabel == "if1")
        {
            ...
            // copy of the bios table - biosTableAttrs
            manager.get().setDefaultBIOSTableAttrsOnIntf(intfLabel, "ipv4");
            addrs.emplace("eth1/v4", std::make_unique<HypIPAddress>(
                                      bus,
                                      sdbusplus::message::object_path(
                                          objectPath.str + "/ipv4/addr0"),
                                      *this, *ifaddrv4, "0.0.0.0",
            ...
            manager.get().setDefaultBIOSTableAttrsOnIntf(intfLabel, "ipv6");
            addrs.emplace("eth1/v6", std::make_unique<HypIPAddress>(
                                      bus,
                                      sdbusplus::message::object_path(
                                          objectPath.str + "/ipv6/addr0"),
                                      *this, *ifaddrv6, "::",
...
std::string HypEthInterface::defaultGateway6(std::string gateway)
{
    ...
    {
        lg2::error("Invalid IPv6 Gateway: {GATEWAY}, Error: {ERROR}", "GATEWAY", gateway, "ERROR", e.what());
    ...
    for (auto& addr : addrs)
    {
        ...
        if (ipObj->type() == HypIP::Protocol::IPv6)
        {
            ...
            {
                lg2::error("Cannot set IPv6 default gateway in DHCP mode to {GATEWAY}", "GATEWAY", gateway);
