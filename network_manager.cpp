...
void Manager::createDefaultNetworkFiles(bool force)
{
    bool create = false;
    try
    {
        ...
        // Don't need to restart the network for factory reset case
        // where force would be true.
        // only restart the network when network manager starts and
        // creates the network file.
        if (!force && create)
        {
            restartNetwork();
        }
