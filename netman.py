...
def ModifyDhcpNtpSetting (confFile, usentp):
    ...
    # Hard coding eth0 for right now.
    rc = call(["ip", "addr", "flush", "eth0"])
...
class NetMan (dbus.service.Object):
    ...
    def SetNtpServer (self, device, ntpservers):
        ...
        ntp_ip = ''
        ntp_entry = ntpservers.split()
        for ntp in ntp_entry:
                ntp_ip = ntp_ip +  ' ' + ntp
    ...
    def UpdateUseNtpField (self, usentp):
        ...
        for configfile in filelist:
            ModifyDhcpNtpSetting(configfile,usentp)
        return 0
