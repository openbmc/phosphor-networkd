...
std::vector<InterfaceInfo> getInterfaces()
{
    ...
    auto cb = [&info](const nlmsghdr& hdr, std::string_view msg) {
        detail::parseInterface(info, hdr, msg);
    };
    ifinfomsg msg{};
...
std::string EthernetInterface::mACAddress(std::string value)
{
    ...
    if (!equal(newMAC, oldMAC))
    {
        ...
        {
            execute("/sbin/fw_setenv", "fw_setenv", "ethaddr1", value.c_str());
        }
    ...
    return MacAddressIntf::mACAddress(value);
}
...
void parseInterface(std::vector<InterfaceInfo>& ret, const nlmsghdr& hdr,
                    std::string_view msg)
{
    ...
    InterfaceInfo info;
    bool set_name = false;
    info.index = ifinfo.ifi_index;
    while (!msg.empty())
    {
        auto [hdr, data] = netlink::extractRtAttr(msg);
        if (hdr.rta_type == IFLA_IFNAME)
        {
            info.name = data.substr(0, strnlen(data.data(), data.size()));
            set_name = true;
        }
        if (hdr.rta_type == IFLA_ADDRESS)
        {
            ...
            info.mac = copyFrom<ether_addr>(data, "Bad interface MAC");
        }
    }
    if (!set_name)
    {
