    ...
    bus(bus), manager(parent), objPath(objPath)
{
    ...
    EthernetInterfaceIntf::linkLocalAddressing(getLinkLocalAddressingFromConf());
...
LinkLocalAddressing EthernetInterface::linkLocalAddressing(LinkLocalAddressing value)
{
    if (value == EthernetInterfaceIntf::linkLocalAddressing())
    {
        return value;
    }

    EthernetInterfaceIntf::linkLocalAddressing(value);
    manager.writeToConfigurationFile();
    return value;
}
