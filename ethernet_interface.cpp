...
void EthernetInterface::createIPAddressObjects() {
  ...
  for (auto& addr : addrs) {
    ...
    std::string ipAddressObjectPath =
        generateObjectPath(addressType, addr.ipaddress, addr.prefix, gateway);
...
void EthernetInterface::iP(IP::Protocol protType, std::string ipaddress,
  ...
  this->addrs.emplace(ipaddress, std::make_shared<phosphor::network::IPAddress>(
...
void EthernetInterface::writeDNSEntries(const ServerList& dnsList,
  ...
  if (!outStream.is_open()) {
    log<level::ERR>("Unable to open the file", entry("FILE=%s", file.c_str()));
    ...
    for (const auto& addr : addrs) {
      if (addr.second->origin() == AddressOrigin::Static) {
        ...
        std::string destination = getNetworkID(
            addressFamily, addr.second->address(), addr.second->prefixLength());
...
std::string EthernetInterface::mACAddress(std::string value) {
  if (!mac_address::validate(value)) {
    log<level::ERR>("MACAddress is not valid.", entry("MAC=%s", value.c_str()));
  ...
  if (!(intMac & mac_address::localAdminMask)) {
    try {
      ...
      log<level::ERR>(
          "Exception occurred during getting of MAC "
